...
class quake3(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        tcp_udp_listener = nlb.add_listener(
            f"TCPUDPListener{app_port}", port=app_port, protocol=elbv2.Protocol.TCP_UDP
        )
        tcp_udp_listener.add_targets(
            f"FargateServiceTargetTCPUDP{app_port}",
            port=app_port,
            protocol=elbv2.Protocol.TCP_UDP,
            targets=[
                service.load_balancer_target(
                    container_name=f"{app_group_l}-container", container_port=app_port
                )
            ],
